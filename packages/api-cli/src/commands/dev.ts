      ...
      try {
        ...
        if (options.docs) {
          try {
            ...
            console.error(`⚠️  Failed to start documentation server:`, error instanceof Error ? error.message : error);
          ...
          reloadTimeout = setTimeout(() => {
            ...
            if (docsServer) {
              try {
                ...
                console.error('⚠️  Failed to update documentation:', error instanceof Error ? error.message : error);
              }
            }
        ...
        if (docsServer) {
          console.log(`   GET    http://${options.host}:${docsPort}/docs        - API documentation (Scalar UI)`);
          console.log(`   GET    http://${options.host}:${docsPort}/openapi.json - OpenAPI specification`);
        }
        console.log('');
        console.log('💡 Tips:');
        ...
        if (docsServer) {
          console.log(`   • Visit http://${options.host}:${docsPort}/docs for interactive API documentation`);
        }
        if (!options.verbose) {
